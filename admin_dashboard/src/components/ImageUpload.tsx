...
interface ImageUploadProps {
  onUpload: (images: File[]) => void; // Prop to handle the uploaded files
}

const ImageUpload: React.FC<ImageUploadProps> = ({ onUpload }) => {
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      const imageFile = Array.from(event.target.files); // Convert FileList to an array
      onUpload(imageFile); // Call the parent callback with the files
    }
  };
  return (
